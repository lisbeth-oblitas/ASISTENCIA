...
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.asistencia.AsistenciaForm
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.actividad.ActividadUI
import pe.edu.upeu.asistenciaupeujc.ui.presentation.screens.asistencia.AsistenciaUI
...
fun NavigationHost(
    ...
    NavHost(
        ...
        composable(Destinations.AsistenciaUI.route){
            AsistenciaUI(navegarEditarAct = {newText->navController.navigate(Destinations.AsistenciaForm.passId(newText))}, navController =navController )
        }
        ...
        composable(Destinations.AsistenciaForm.route, arguments = listOf(navArgument("asisId"){
            defaultValue="actId"
        })){
                navBackStackEntry -> var asisId=navBackStackEntry.arguments?.getString("asisId")
            requireNotNull(asisId)
            ActividadForm(text = asisId, darkMode = darkMode, navController =navController )
        }
        ...
        composable(Destinations.AsistenciaUI.route){
            AsistenciaxUI(navegarEditarAct = {newText->navController.navigate(Destinations.AsistenciaForm.passId(newText))}, navController =navController )
        }

        composable(Destinations.AsistenciaForm.route, arguments = listOf(navArgument("matId"){
            defaultValue="matId"
        })){
                navBackStackEntry -> var matId=navBackStackEntry.arguments?.getString("matId")
            requireNotNull(matId)
            AsistenciaForm(text = matId, darkMode = darkMode, navController =navController )
